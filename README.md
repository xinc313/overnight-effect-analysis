...
7. **TQQQ 与连败处理(第三个标签页)**:信号在 TQQQ 上同样成立(前夜跌后 21.5bp、t=3.56);连跌越深下一夜越强(连跌 3 夜后 QQQ +18.9bp、t=3.00);买入后开盘亏损时,扛到收盘没有期望补偿(日内 −5bp、t=−1.1),回测显示「开盘照卖、信号在就再进」的基准政策夏普最高,「亏后休息」最差。TQQQ 条件策略 16 年 ×18.4(含 1bp 成本),接近 QQQ 买入持有,但回撤 −48% vs −82%;注意 TQQQ 样本只含牛市。

## 最终模型(第五个标签页)

只在「昨夜隔夜下跌、且今晚不含周末」时持有 QQQ 隔夜段;连跌加仓 1x/1.5x/2x;波动率刹车(60 夜中位数 ÷ 当前波动,限 0.5–2)后总仓位封顶 2x;次日开盘无条件卖出。**全样本(2000–2026,含 1bp/边成本)年化 +10.4%、夏普 1.15、最大回撤 −17.7%;样本外(2016–2026)夏普 1.06**。27 个年度 22 个为正;2025/2026 连续为负是首次,已列入失效监控清单(滚动 3 年夏普、信号夜胜率、连败包络等)。
...
python scripts/final_model.py       # 最终模型整体回测 + 导出第五个标签页数据
```
